...
:::{warning}
Currently, Juju does not support any kind of isolation between Juju models created in a GCP project
and other resources in the same project. This could make the cluster difficult to manage if you have
...
* `credential-name` — Your choice of name that will help you identify the credential set, e.g. `my-gcp-credential`.
* `region` — The region the credential is tied to e.g. `us-east-1`.
  Since GCP users are not tied to a specific GCP region, this can be left blank.
* `auth type` -- The method of authentication. Select `jsonfile` here.
* `Path` -- The path to the Service Account key file, which corresponds to `~/.local/share/juju/juju-sa-creds.json`.
...
Take note of the project number shown in the output (here `12345678901`), and use that to grant
the required role to the default Service Account:
...
Bootstrap GKE using the `gcloud container cluster` command with your choice of memorable name for the
