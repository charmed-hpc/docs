...
A typical Charmed HPC deployment will use a mix of high-performance and GPU-accelerated compute instances for cluster compute
...
* `region` — a default region that is most convenient to deploy your controller and applications, e.g. `us-east-1`. Note that credentials are not region-specific.
...
* `secret-key` — The secret access key generated in the previous section, typically an alphanumeric string with format `xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx/xxxxxxxx`.
  ...
  For security reasons, characters will NOT be displayed when entering this field. However, any input
