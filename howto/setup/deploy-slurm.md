...
1. You can get the node name of a compute node by substituting the forward slash `/` character in the
   ...
   You can use `juju status`{l=shell} to list all currently deployed compute nodes.
2. For example, to list all the units that belong to the slurmd application, run:
...
compute      up   infinite      1   idle slurmd-0
:::
