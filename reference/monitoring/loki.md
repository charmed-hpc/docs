...
The following table lists all the charms on Charmed HPC that expose logs to Loki, and
the corresponding query to see the exported logs in the Grafana UI. Follow the
[Visualize log data](https://grafana.com/docs/loki/latest/visualize/grafana/#grafana-explore) tutorial
from the Grafana documentation to know where to query for Loki logs.
...
`grafana-agent`, which will make it ignore such log files:
